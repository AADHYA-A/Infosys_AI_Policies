...
try:
    QUANTUM_MODEL_PATH = r"C:\Users\Aadhya\Downloads\Infosys_AI_Policies\quantum_nlp_models.pkl"
...
def agriculture_page(request: Request, query: str = None):
    results = search_policies(query, top_k=5) if query else None
    return templates.TemplateResponse("agriculture.html", {"request": request, "results": results, "query": query})

@app.get("/women", response_class=HTMLResponse)
def women_page(request: Request, query: str = None):
    results = search_policies(query, top_k=5) if query else None
    return templates.TemplateResponse("women.html", {"request": request, "results": results, "query": query})

@app.get("/employment", response_class=HTMLResponse)
def employment_page(request: Request, query: str = None):
    results = search_policies(query, top_k=5) if query else None
    return templates.TemplateResponse("employment.html", {"request": request, "results": results, "query": query})
